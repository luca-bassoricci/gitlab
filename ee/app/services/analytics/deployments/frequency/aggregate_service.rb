    ...
    module Frequency
      ...
      class AggregateService < BaseContainerService
        ...
        def execute
          ...
          # TODO: For performance optimization, we should count in PostgreSQL, instead of Ruby.
        ...
        def validate
          ...
          unless VALID_INTERVALS.include?(interval)
            return error(_("This interval is not supported"), :bad_request)
          end

          unless for_project?
            # TODO: We will extend this service for group-level
            return error(_("Only project level aggregation is supported"), :not_implemented)
          end

          unless can?(current_user, :read_cd_dora_analytics, container)
            return error(_("You do not have permission to access deployment frequencies"), :forbidden)
          end
