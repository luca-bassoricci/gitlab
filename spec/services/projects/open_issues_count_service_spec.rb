...
RSpec.describe Projects::OpenIssuesCountService, :use_clean_rails_memory_store_caching do
  ...
  describe '#count' do
    context 'when user is nil' do
      it 'does not include confidential issues or issues created by banned users in the issue count' do
    ...
    context 'when user is an admin' do
      ...
      it 'includes confidential issues in the issue count' do
        create(:issue, :opened, project: project)
        create(:issue, :opened, confidential: true, project: project)
        create(:issue, :opened, author: banned_user, project: project)
      ...
      it 'uses total_open_issues_count cache key' do
        expect(described_class.new(project, admin).cache_key_name).to eq('total_open_issues_count')
      end
    end
    ...
    context 'when user is provided' do
      ...
      context 'when user can read confidential issues' do
        ...
        it 'uses total_open_issues_without_banned_count cache key' do
          expect(described_class.new(project, user).cache_key_name).to eq('total_open_issues_without_banned_count')
        end
      end
      ...
      context 'when user cannot read confidential issues' do
        ...
        it 'uses public_open_issues_count cache key' do
          expect(described_class.new(project, user).cache_key_name).to eq('public_open_issues_without_banned_count')
        end
      end
    end
    ...
    describe '#refresh_cache' do
      ...
      context 'when cache is empty' do
        it 'refreshes cache keys correctly' do
          ...
          expect(Rails.cache.read(subject.cache_key(described_class::TOTAL_COUNT_WITHOUT_BANNED_KEY))).to eq(3)
      ...
      context 'when cache is outdated' do
        ...
        it 'refreshes cache keys correctly' do
          ...
          expect(Rails.cache.read(subject.cache_key(described_class::PUBLIC_COUNT_WITHOUT_BANNED_KEY))).to eq(3)
          expect(Rails.cache.read(subject.cache_key(described_class::TOTAL_COUNT_WITHOUT_BANNED_KEY))).to eq(5)
