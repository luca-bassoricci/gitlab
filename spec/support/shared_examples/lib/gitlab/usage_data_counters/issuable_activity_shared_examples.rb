...
RSpec.shared_examples 'a daily tracked issuable snowplow and service ping events' do
  ...
  context 'with route_hll_to_snowplow_phase2 enabled' do
    before do
    ...
    it '' do
      ...
      expect_snowplow_event(category: 'issues_edit', action: action, user: user1, namespace: namespace, project: project)
    end
  end

  context 'with route_hll_to_snowplow_phase2 disabled'
end
