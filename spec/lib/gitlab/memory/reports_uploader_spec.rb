...
RSpec.describe Gitlab::Memory::ReportsUploader, :aggregate_failures do
  ...
  let(:uploader) { described_class.new(gcs_key: gcs_key, gcs_project: gcs_project, gcs_bucket: gcs_bucket) }
  ...
  describe '#upload' do
    ...
    it 'calls fog, logs upload requested and success with duration' do
      expect(Gitlab::AppLogger)
        ...
        .with(hash_including(:pid, :worker_id,
                             message: "Diagnostic reports", perf_report_status: "upload requested",
      ...
      expect(Gitlab::AppLogger)
        .to receive(:info)
        .with(hash_including(:pid, :worker_id, :duration_s,
    ...
    context 'when Google API responds with an error' do
      ...
      let(:uploader) { described_class.new(gcs_key: gcs_key, gcs_project: gcs_project, gcs_bucket: invalid_bucket) }
      ...
      it 'logs error raised by Fog and do not re-raise' do
        ...
        expect(Gitlab::ErrorTracking)
          ...
          .with(instance_of(Google::Apis::ClientError),
