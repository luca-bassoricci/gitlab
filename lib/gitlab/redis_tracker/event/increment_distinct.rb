  ...
  module RedisTracker
    module Event
      class IncrementDistinct
        ...
        def initialize(event:, values:, context: nil, date: Date.current)
          @values = values
          @definition = Gitlab::RedisTracker::Event::Definition.new(event: event)
          @redis_key = Gitlab::RedisTracker::Event::Key.build(:redis_increment_distinct_key, event: event, date: date, context: context)
        end
